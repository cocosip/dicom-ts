...
/**
 * Modality Sequence LUT — implements IModalityLUT via ModalityLUTSequence item.
 *
 * Reference: fo-dicom/FO-DICOM.Core/Imaging/LUT/ModalitySequenceLUT.cs
 */
export class ModalitySequenceLUT implements IModalityLUT {
  ...
  constructor(modalityLutSequenceItem: DicomDataset, signed: boolean) {
    this._signed = signed;
    this._item = modalityLutSequenceItem;
    this.recalculate();
  }

  /** Always recalculate — returns false so PrecalculatedLUT rebuilds each time. */
  get isValid(): boolean {
    return false;
  }
...
// ---------------------------------------------------------------------------
// Helpers to extract typed values from DicomElement buffers
// ---------------------------------------------------------------------------
