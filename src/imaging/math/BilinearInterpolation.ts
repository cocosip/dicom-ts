...
export class BilinearInterpolation {
  ...
  /**
   * Rescale a 32-bit color image (4 bytes per pixel, RGBA interleaved) using bilinear interpolation.
   */
  static rescaleColor32(
    input: Uint8Array,
    inputWidth: number,
    inputHeight: number,
    outputWidth: number,
    ...
    for (let y = 0; y < outputHeight; y++) {
      ...
      const dy1 = oy0 - oy1;
      const dy2 = 1.0 - dy1;
      const yo0 = outputWidth * y * 4;
      const yo1 = inputWidth * oy1 * 4;
      const yo2 = inputWidth * oy2 * 4;
      for (let x = 0, px = 0; x < outputWidth; x++) {
        const ox0 = x * xF;
        const ox1 = ox0 | 0;
        const ox2 = ox1 === xMax ? ox1 : ox1 + 1;
