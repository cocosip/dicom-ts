/**
 * DICOM JPEG Extended (Process 2 & 4) codec — plugin-path adapter pattern.
 *
 * Transfer syntax: 1.2.840.10008.1.2.4.51
 *
 * Supports 8-bit and 12-bit samples.  Callers supply a `DicomJpegAdapter`.
 */
...
export class DicomJpegProcess2_4Codec implements IDicomCodec {
  ...
  private decodeSingleFrame(pixelData: DicomPixelData, frame: number, params: DicomJpegParams): IByteBuffer {
    if (pixelData.bitsStored > 12) {
      throw new Error(
        `JPEG Extended (Process 2 & 4) supports up to 12-bit samples; ` +
