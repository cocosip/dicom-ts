...
import {
  ...
  type DicomJpegExtendedParameters,
...
export interface IDicomJpegExtendedAdapter {
...
export class DicomJpegProcess2_4Codec implements IDicomCodec {
  readonly transferSyntax: DicomTransferSyntax = DicomTransferSyntax.JPEGProcess2_4;
  readonly parameters: Readonly<Required<DicomJpegCodecParameters>>;
  private readonly adapter: IDicomJpegExtendedAdapter;

  constructor(adapter: IDicomJpegExtendedAdapter, parameters: DicomJpegExtendedParameters = {}) {
  ...
  private validateBitDepth(pixelData: DicomPixelData, frame: number): void {
    ...
    if (pixelData.bitsStored > 12) {
      throw new Error(`${prefix} JPEG Extended (Process 2 & 4) supports up to 12-bit samples`);
    }
    if (pixelData.bitsAllocated > 16) {
      throw new Error(`${prefix} JPEG Extended (Process 2 & 4) expects BitsAllocated <= 16`);
    }
  }

  private errorPrefix(frame: number, pixelData: DicomPixelData): string {
    return `[JPEG Extended uid=${this.transferSyntax.uid.uid} frame=${frame} bitsStored=${pixelData.bitsStored}]`;
  }
}
