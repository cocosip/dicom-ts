 ...
 */
export class DicomJpegLsLosslessCodec implements IDicomCodec {
  readonly name = "JPEG-LS Lossless";
  readonly transferSyntax = DicomTransferSyntax.JPEGLSLossless;

  getDefaultParameters(): DicomJpegLsParams {
    const parameters = new DicomJpegLsParams();
    parameters.allowedError = 0;
    return parameters;
  }
  ...
  private decodeFrame(pixelData: DicomPixelData, frameIndex: number, parameters: DicomJpegLsParams): IByteBuffer {
    ...
    if (decoded.near !== 0) {
      throw new Error(
        `JPEG-LS Lossless decode expected NEAR=0; got ${decoded.near} [frame=${frameIndex}, syntax=${this.transferSyntax.uid.uid}]`,
      );
    }
    ...
    if (parameters.interleaveMode !== 0 && parameters.interleaveMode !== decoded.interleaveMode) {
      throw new Error(
        `JPEG-LS Lossless decode interleave mismatch: expected=${parameters.interleaveMode}, actual=${decoded.interleaveMode} [frame=${frameIndex}, syntax=${this.transferSyntax.uid.uid}]`,
      );
    }
  ...
  private resolveParams(parameters: DicomCodecParams | null): DicomJpegLsParams {
    ...
    resolved.allowedError = normalizeAllowedError(source.allowedError, 0);
    ...
    if (resolved.interleaveMode !== 0) {
      throw new Error(
        `JPEG-LS Lossless currently supports interleaveMode=0 only; got ${resolved.interleaveMode} [syntax=${this.transferSyntax.uid.uid}]`,
      );
    }
