import { readSync } from "node:fs";
import type { IByteBuffer } from "./IByteBuffer.js";

/**
 * Byte buffer backed by a seekable stream (file descriptor).
 * Reference: fo-dicom/FO-DICOM.Core/IO/Buffer/StreamByteBuffer.cs
 */
export class StreamByteBuffer implements IByteBuffer {
  readonly isMemory = false as const;
  readonly stream: StreamLike | number;
  readonly position: number;
  readonly size: number;
  private readonly _fd: number;

  constructor(stream: StreamLike | number, position: number, length: number) {
    const fd = resolveFd(stream);
    if (fd === null) {
      throw new Error("StreamByteBuffer requires a stream with a valid file descriptor");
    }
    this.stream = stream;
    this.position = Math.max(0, position);
    this.size = Math.max(0, length);
    this._fd = fd;
  }

  get data(): Uint8Array {
    return this.getByteRange(0, this.size);
  }

  getByteRange(offset: number, count: number): Uint8Array {
    if (count <= 0) return new Uint8Array(0);
    if (offset < 0) throw new RangeError(`Offset ${offset} must be >= 0`);
    if (offset >= this.size) return new Uint8Array(0);

    const available = Math.min(count, this.size - offset);
    const buffer = Buffer.allocUnsafe(available);
    readFully(this._fd, buffer, this.position + offset);
    return buffer;
  }
}

type StreamLike = { fd?: number | null };

function resolveFd(stream: StreamLike | number): number | null {
  if (typeof stream === "number") return stream;
  if (!stream) return null;
  const fd = stream.fd;
  return typeof fd === "number" ? fd : null;
}

function readFully(fd: number, target: Buffer, position: number): void {
  let read = 0;
  while (read < target.length) {
    const n = readSync(fd, target, read, target.length - read, position + read);
    if (n === 0) break;
    read += n;
  }
  if (read < target.length) {
    target.subarray(read).fill(0);
  }
}
