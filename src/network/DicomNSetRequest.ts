...
export class DicomNSetRequest extends DicomRequest {
  onResponseReceived?: (request: DicomNSetRequest, response: DicomNSetResponse) => void;
  ...
  constructor(commandOrClassUid: DicomDataset | DicomUID, requestedSopInstanceUid?: DicomUID) {
    ...
    super(DicomCommandField.NSetRequest, commandOrClassUid);
  ...
  protected override postResponse(response: DicomResponse): void {
    this.onResponseReceived?.(this, response as DicomNSetResponse);
  }
}
